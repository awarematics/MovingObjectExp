...
function createQueryRange() {
	var order = document.getElementById("order_range").value;
	index = order.lastIndexOf("\\");
	order = order.substring(index + 1);
	var queryString = "order_range=" + order;
	return queryString;
}
function doRequestUsingGET_Result() {
	createXMLHttpRequest();
	var queryString = "GetAndPostLayer?";
...
function doRequestUsingGET_Range() {
	createXMLHttpRequest();
	var queryString = "GetAndPostRange?";
	queryString = queryString + createQueryRange()
	+ "&timeStamp=" + new Date().getTime();
	xmlHttp.open("GET", queryString, true);
...
function doRequestUsingPOST_Result() {
	createXMLHttpRequest();
	var url = "GetAndPostLayer?timeStamp=" + new Date().getTime();
...
function doRequestUsingPOST_Range() {
	createXMLHttpRequest();
	var url = "GetAndPostRange?timeStamp=" + new Date().getTime();
	var queryString = createQueryRange();
	xmlHttp.open("POST", url, true);
	xmlHttp.onreadystatechange = handleStateChange_Range;
	xmlHttp.setRequestHeader("Content-Type",
		"application/x-www-form-urlencoded;");
...
function parseRange() {
	document.getElementById('serverRange').innerHTML = xmlHttp.responseText;
	var kkk1 = xmlHttp.responseText;
	rangequery(kkk1);
	var responseDiv = document.getElementById("serverRange");
	if (responseDiv.hasChildNodes()) {
		responseDiv.removeChild(responseDiv.childNodes[0]);
	}
	var responseText = document.createTextNode(xmlHttp.responseText);
