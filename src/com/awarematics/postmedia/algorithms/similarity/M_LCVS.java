...
public class M_LCVS {
	...
	private Polygon genFoVArea(double x, double y, FoV fov, int unitAngle ) {
		...
		{
			double x_temp =  x + fov.getDistance() * Math.sin(Math.toRadians(fov.getDirection() - (fov.getViewAngle() ) / 2 +unitAngle*i )) * times;
			double y_temp =  y + fov.getDistance() * Math.cos(Math.toRadians(fov.getDirection() - (fov.getViewAngle() ) / 2 + unitAngle*i )) * times;
